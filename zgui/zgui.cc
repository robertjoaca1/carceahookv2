...
void zgui::checkbox(const char *id, bool &value) {
    ...
    window_ctx.render.emplace_back(zgui_control_render_t{{draw_pos.x + 15, draw_pos.y + 1 }, zgui_render_type::zgui_text, global_colors.color_text, id_split[0], vec2{0, 0}, font});
    ...
    utils::misc::push_cursor_pos(vec2{cursor_pos.x + 13 + text_width + global_config.item_spacing, cursor_pos.y});
    utils::misc::push_cursor_pos(vec2{cursor_pos.x, cursor_pos.y + 10 + global_config.item_spacing});
...
void zgui::begin_groupbox(std::string_view title, const vec2 size) {
	...
	/*functions.draw_filled_rect(draw_pos.x + 1, draw_pos.y + 18, size.x - 2, size.y - 19, global_colors.color_groupbox_bg);
    functions.draw_filled_rect(draw_pos.x + 1, draw_pos.y + 16, size.x - 2, 3, global_colors.control_outline);
    functions.draw_filled_rect(draw_pos.x + 1, draw_pos.y + 17, size.x - 2, 1, global_colors.control_active_or_clicked);*/
...
bool zgui::begin_window(std::string_view title, const vec2 default_size, const unsigned long font, const int flags) {
	...
	if (window_ctx.opened || window_ctx.alpha > 0) {
		...
		//functions.draw_filled_rect(window_ctx.position.x + 4, window_ctx.position.y + 13, window_ctx.size.x - 8, 1, global_colors.window_border_color);
